...
Opt_B_M=A-diag(opt_lambda_M);
lambda_min_opt_B_M=min(eig(Opt_B_M))
% % Time Graph
% G=gsp_ring(n,1);
% S=full(G.L);
% [U,D]=eig(S);
...
estimated_z=Cov_z*inv(Cov_x)*x';
figure;
sgtitle("Stationary Approximation of a Process on a 2D Grid");
subplot(2,2,1);
...
title("z Realization");
subplot(2,2,3);
gsp_plot_signal(G,x-z,param);
title("w Realization");
subplot(2,2,4);
gsp_plot_signal(G,estimated_z,param);
